...
Lattice VM is built upon several pieces of technology inherited from UTM,
layered to provide compatibility across host configurations. Internal type names
retain their `UTM` prefix for source and VM-format compatibility.
...
The QEMU backend uses UTM's maintained QEMU fork, which provides:
...
`QEMUHelper` is an XPC helper with an App Sandbox separate from the Lattice VM
application. This adds isolation but requires security-scoped access when
passing disks, shared directories, sockets, and other host resources.
...
The macOS creation wizard can query restore-image metadata, select macOS 12 or
newer, and download the chosen IPSW from an Apple-controlled HTTPS CDN. Lattice
VM validates the HTTP response and IPSW archive header, removes download quarantine,
sets permissions suitable for Apple's out-of-process installation service, and
then obtains a supported hardware model from `VZMacOSRestoreImage`. Completed
images are cached locally and are never stored in the repository.

Windows and Linux selectors open official vendor download pages. Those vendors
...
QEMU configurations use `UTMQemuConfiguration`. Keep migrations compatible
with both formats.

## CocoaSpice

Lattice VM uses SPICE with QEMU for USB forwarding, multiple displays, guest
clipboard integration, and dynamic resolution. [CocoaSpice][2] bridges SPICE
framebuffers to Metal textures.
...
The frontend is primarily SwiftUI, with shared views where platform behavior
allows it and AppKit/UIKit components where lower-level control is required.
