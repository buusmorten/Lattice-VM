...
Lattice VM inherits UTM's QEMU, SPICE, and native-library dependency system.
The build scripts create Xcode-compatible frameworks from Autoconf and Meson
projects. Keep dependency patches suitable for upstreaming whenever possible.

The checked-in source does not contain redistributable operating-system
installers. The macOS catalog uses third-party metadata only and downloads
restore images from Apple; Windows and Linux links lead to their respective
...
Dependency scripts, sysroot directories, schemes, and generated files may
retain `UTM` names because they are inherited build interfaces. The public
project is `LatticeVM.xcodeproj`, and the application product is Lattice VM.
