...
Lattice VM inherits UTM's graphics architecture and translation layers. The
available renderer depends on the backend, guest drivers, host platform, and
build configuration; selecting a renderer does not guarantee guest 3D support.
...
†: Future work that is not currently available in Lattice VM.
...
[gfxstream][3] is an alternative library that allows the guest to serialize
OpenGL and Vulkan commands, pass them through a communication channel ("pipe")
to the host, and have the host evaluate the calls. It differs from
virglrenderer in that there is no intermediate translation. It is not currently
exposed as a Lattice VM renderer.

## ANGLE

[ANGLE][4] is an implementation of OpenGL ES on top of other graphics APIs.
The inherited graphics stack supports three ANGLE backends:
...
[MoltenVK][5] translates Vulkan to Metal because Apple devices do not support
Vulkan natively. MoltenVK is currently not used by Lattice VM.
...
Lattice VM uses SPICE as its QEMU frontend. Remote SPICE sessions transfer
pixel-buffer updates and do not support the same GPU acceleration as local EGL
canvas rendering.
