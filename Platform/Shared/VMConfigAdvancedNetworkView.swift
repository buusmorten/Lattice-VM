...
struct IPConfigurationSection: View {
    ...
    private var additionalSearchDomains: Binding<String> {
        Binding {
            config.vlanDnsSearchDomains.joined(separator: ", ")
        } set: { value in
            config.vlanDnsSearchDomains = value
                .split(separator: ",")
        ...
        Group {
            ...
            if config.mode == .emulated {
                ...
                DefaultTextField("DNS Search Domains", text: $config.vlanDnsSearchDomain.bound)
                    .keyboardType(.asciiCapable)
                DefaultTextField("Additional Search Domains", text: additionalSearchDomains, prompt: "corp.example, dev.example")
                    .keyboardType(.asciiCapable)
                DefaultTextField("Local Domain", text: $config.localDomain.bound, prompt: "dev.lattice")
                    .keyboardType(.asciiCapable)
                Text("QEMU supports one IPv4 and one IPv6 DNS server. Search domains are tried in the displayed order.")
                    .font(.caption)
