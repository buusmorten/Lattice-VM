...
let productName = "Lattice VM SE"
#elseif WITH_REMOTE && !os(visionOS)
let productName = "Lattice VM Remote"
#else
let productName = "Lattice VM"
#endif
            ...
            if !Main.jitAvailable {
                data.busyWorkAsync {
                    ...
                    if !UTMCapabilities.current.contains(.hasHypervisorSupport) {
                        throw NSLocalizedString("Your version of iOS does not support running VMs while unmodified. You must either run Lattice VM while jailbroken or with a remote debugger attached. See https://getutm.app/install/ for more details.", comment: "ContentView")
                    }
                }
            }
            #endif
