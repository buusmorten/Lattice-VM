...
struct VMConfigPortForwardForm: View {
    ...
    private var localhostOnly: Binding<Bool> {
        Binding {
            !forward.isExposedToLAN
        } set: { isLocal in
            forward.hostAddress = isLocal ? "127.0.0.1" : "0.0.0.0"
        }
    }
    ...
    var body: some View {
        Group {
            ...
            NumberTextField("Host Port", number: $forward.hostPort, prompt: "1234")
                .help("Host Port")
            Toggle("Localhost only", isOn: localhostOnly)
                .help("When enabled, the forwarded service is reachable only from this Mac.")
            if forward.isExposedToLAN {
                Label("This service will listen on every host interface and may be reachable from your LAN.", systemImage: "exclamationmark.triangle.fill")
                    .font(.caption)
