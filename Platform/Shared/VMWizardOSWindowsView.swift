...
struct VMWizardOSWindowsView: View {
    private struct WindowsRelease: Identifiable {
        let id: String
        let title: String
        let downloadURL: URL
    }
    ...
    private let windowsReleases = [
        WindowsRelease(id: "11", title: "Windows 11", downloadURL: URL(string: "https://www.microsoft.com/software-download/windows11")!),
        WindowsRelease(id: "10", title: "Windows 10", downloadURL: URL(string: "https://www.microsoft.com/software-download/windows10ISO")!),
        WindowsRelease(id: "server-2025", title: "Windows Server 2025", downloadURL: URL(string: "https://www.microsoft.com/evalcenter/download-windows-server-2025")!)
    ]

    @ObservedObject var wizardState: VMWizardState
    @State private var isFileImporterPresented: Bool = false
    @State private var selectedWindowsRelease = "11"
        ...
        VMWizardContent("Windows") {
            ...
            Section {
                ...
                if wizardState.isWindows10OrHigher {
                    Picker("Windows Version", selection: $selectedWindowsRelease) {
                        ForEach(windowsReleases) { release in
                            Text(release.title).tag(release.id)
                        }
                    }
                    if let release = windowsReleases.first(where: { $0.id == selectedWindowsRelease }) {
                        Link(destination: release.downloadURL) {
                            Label("Download \(release.title) from Microsoft…", systemImage: "arrow.down.circle")
                        }
                        .buttonStyle(.borderless)
                    }
                    Text("Microsoft generates temporary ISO links on its download pages. Download the ISO, then select it below to continue installation.")
                        .font(.caption)
