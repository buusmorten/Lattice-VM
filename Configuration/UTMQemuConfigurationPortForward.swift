...
extension UTMQemuConfigurationPortForward {
    ...
    var isExposedToLAN: Bool {
        guard let hostAddress, !hostAddress.isEmpty else { return true }
        return hostAddress == "0.0.0.0" || hostAddress == "::"
    }

    init(preset: Preset, localhostOnly: Bool = true) {
        self.init()
        hostAddress = localhostOnly ? "127.0.0.1" : "0.0.0.0"
        hostPort = preset.port
        guestPort = preset.port
    }
}
